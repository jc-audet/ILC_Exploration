...
def get_train_test_loaders(path, batch_size, num_workers,
    ...
    train_ds = ColoredMNIST(root='./data', env='all_train',
                        transform=Compose([
                                  ToTensor(),
                                  Normalize((0.1307, 0.1307, 0.), (0.3081, 0.3081, 0.3081))
                        ]))

    test_ds = ColoredMNIST(root='./data', env='test', 
                      transform=Compose([
                        ToTensor(),
                        Normalize((0.1307, 0.1307, 0.), (0.3081, 0.3081, 0.3081))
                        ]))
    ...
    targets = np.zeros(len(train_ds))
    for i in range(len(train_ds)):
      _, targets[i] = train_ds[i]
    targets = np.array(targets)

    label_rng = np.random.RandomState(seed=random_seed)
    print(f'RANDOM LABELS FRACTION: {random_labels_fraction}')
    n_random = int(round(random_labels_fraction * len(train_ds)))
    rnd_idxs = label_rng.choice(np.arange(len(train_ds)),
                                size=n_random,
                                replace=False)
    ...
    for rnd_idx in rnd_idxs:
        targets[rnd_idx] = label_rng.randint(10)

    mislabeled_idxs, = np.where(np.array(original_targets) != np.array(targets))
    ...
    train_loader = DataLoader(train_ds, batch_size=batch_size,
                       ...
                       shuffle=True,
                       drop_last=True)
    mislabeled_train_loader = DataLoader(train_ds, batch_size=batch_size,
                       num_workers=num_workers,
                       pin_memory=pin_memory,
